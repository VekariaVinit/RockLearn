...
// Controller to handle fetching all files and zip link for the repository
async function getAllFiles(req, res) {
    const repoName = req.params.repoName;
    try {
        const allFiles = await fetchAllFiles(repoName);
        ...
        const repoDownloadLink = `https://github.com/${GITHUB_USERNAME}/${repoName}/archive/refs/heads/main.zip`;

        res.json({ directories, repoName, repoDownloadLink });
    } catch (error) {
        console.error('Error fetching files:', error);
        res.status(500).json({ message: 'Error fetching files' });
    }
...
async function getFileContent(req, res) {
    ...
    try {
        const content = await fetchFileContent(repoName, filePath.trim());
        res.json({ content });
    } catch (error) {
        console.error(`Error fetching content for file ${filePath}:`, error);
        res.status(500).json({ message: 'Error fetching file content' });
